 ...
 * js加载demo
 */
 var math = require('./math.js'); //commonjs规范，会将math.js打包进主文件（loadjs.js）
...
$('#aaa').on('click', function(){
    //var math2 = require('./math2.js'); //同步加载，打包时会直接打包进来，无需点击按钮时加载
    ...
    require.ensure(['./math3.js'], function(require){  //在需要的时候才下载依赖的模块，[]内为依赖文件没有依赖写空数组，依赖文件不能写在入口配置里面，依赖文件会和回调函数里面的require文件一起打包
        ...
        var math2 = require('./math2.js'); //点击时才加载，math3.js和math2.js会打包为一个文件math2.chunk.js
        var sum = math2.add(1, 2);
        console.log(sum);
    }, 'math2'); //需指定publicpach, math2为chunkFilename，指定该chunk的名称，如果这个chunk名已经存在了，则将本次依赖的模块合并到已经存在的chunk中
